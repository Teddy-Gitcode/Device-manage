...
export function DashboardShell({ devices }: Props) {
  ...
  return (
    <div className="page-fade">
      ...
      {/* ── 1. KPI strip ── */}
      <KpiTiles devices={devices} />

      {/* ── 2. Action items + toner to order ── */}
      <div className="dash-priority-row" style={{ marginBottom: 16 }}>
        <ActionItems devices={devices} />
        <TonerToOrder devices={devices} />
      </div>

      {/* ── 3. Fleet by location ── */}
      <FleetByLocation devices={devices} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '20px 0', color: 'var(--status-success-fg)' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: 'var(--neutral-fg-3)', marginTop: 2 }}>
                All {devices.length} devices are online with no active alerts
              </div>
            </div>
          </div>
        ) : (
